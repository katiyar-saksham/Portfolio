...
function animateSectionsOnScroll() {
    gsap.utils.toArray(['#about', '#move', '#skills', '#others', '#projects', '#contact']).forEach(section => {
        gsap.from(section, {
            y: 100,
            opacity: 0,
            duration: 1,
...
function initBoardHoverEffect() {
    ...
    if (!boardsContainer || boards.length === 0) return;

    boardsContainer.addEventListener("mousemove", (e) => {
